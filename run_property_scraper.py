# run_property_scraper.py; main script 2
import logging

import pandas as pd

import scraper.geocoding
from scraper import db_manager, utils
from scraper.property_details_scraper import scrape_property_details

logging.basicConfig(
    level=logging.INFO
)  # Enable logging INFO level for more detailed output
...
def main():
    ...
    if conn:
        ...
        finn_codes_to_scrape = db_manager.fetch_finn_codes_from_db(
            conn, select_all=True
        )
        properties_data = []
        for i, finn_code_data in enumerate(finn_codes_to_scrape):
            finn_code = finn_code_data[
                0
            ]  # Assuming finn_code is the first element in the tuple
            property_data = scrape_property_details(finn_code, config)
            if property_data:
                property_data["finn_code"] = (
                    finn_code  # Ensure finn_code is in property data
                )

                # Geocode address and add lat/long
                address_to_geocode = property_data.get("address")
                if address_to_geocode:
                    geocode_result = scraper.geocoding.geocode_address(
                        address_to_geocode
                    )
                    if geocode_result:
                        ...
                        logger.info(
                            f"Geocoded address for Finn code {finn_code}: Lat={property_data['latitude']}, Long={property_data['longitude']}"
                        )
                    else:
                        logger.warning(
                            f"Geocoding failed for address: {address_to_geocode} (Finn code: {finn_code})"
                        )
                else:
                    logger.warning(
                        f"No address found for Finn code {finn_code}, cannot geocode."
                    )
...
def geocode_existing_properties():
    ...
    if conn:
        ...
        for property_record in properties_to_geocode:
            ...
            address_to_geocode = property_record[
                2
            ]  # Assuming address is the third element (after finn_code, scrape_status)

            if address_to_geocode:
                geocode_result = scraper.geocoding.geocode_address(address_to_geocode)
                if geocode_result:
                    latitude = geocode_result["latitude"]
                    longitude = geocode_result["longitude"]
                    db_manager.update_property_location(
                        conn, finn_code, latitude, longitude
                    )
                    logger.info(
                        f"Geocoded Finn code {finn_code}: Address='{address_to_geocode}', Lat={latitude}, Long={longitude}"
                    )
                else:
                    logger.warning(
                        f"Geocoding failed for address: '{address_to_geocode}' (Finn code: {finn_code})"
                ...
                logger.warning(
                    f"No address found in database for Finn code {finn_code}, cannot geocode."
                )
